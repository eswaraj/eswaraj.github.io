...
$(document).ready(function(){
	$.ajax({
		type: "GET",
		url:"http://demo5303989.mockable.io/ajax/amenities/get",
		contentType: "application/json; charset=utf-8",
		dataType: "JSON",
		async: false,
		success: function(data){
			var filter_list = '<a href="#!" onclick="getData(0)">Show All</a><br>';
			for(var i=0;i < data.length; i++){
				filter_list += '<a href="#!" class = "analytical_filter_options" onclick="getData('+data[i].id+')">'+data[i].name+'</a>';
				chart_data[data[i].id] = null;
			}
			//$('#filter_list').html(filter_list);   
